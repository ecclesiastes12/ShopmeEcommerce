...
//	}
	...
	//constructor modified because moduleURL is now set in PagingAndSortingArgumentResolver class
	...
	public void updateModelAttributes(int pageNum, Page<?> page) {
		...
		//NB code now moved to  PagingAndSortingArgumentResolver class
...
//	}
	...
	public void listEntities(int pageNum, int pageSize, SearchRepository<?, Integer> repo) {
		...
		//code same as code in createPageable method
//		Sort sort = Sort.by(sortField);
//		
//		//sortField now accessed by helper.getSortField
//		//Sort sort = Sort.by(helper.getSortField());
...
//		Pageable pageable = PageRequest.of(pageNum - 1, pageSize, sort);
		...
		//checks if keyword is not empty or not null
		if (keyword != null) {
			//returns the search results if keyword is not null
